...
from model import embedding, fnet
...
class FNetLRASingle(nn.Module):
    def __init__(self, args) -> None:
        super(FNetLRASingle, self).__init__()
        self.embedding = embedding.FNetEmbedding(args.pe_type, 
                                                      args.pooling_type, 
                                                      args.vocab_size, 
                                                      args.max_seq_len, 
                                                      args.embed_size, 
                                                      args.embed_drop_prob)
        self.fnet = fnet.FNet(args)
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        encoded = self.fnet(embeded)
        classified = self.classifier(encoded)

        return classified


class FNetLRADual(nn.Module):
    def __init__(self, args) -> None:
        super(FNetLRADual, self).__init__()
        self.embedding = embedding.FNetEmbedding(args.pe_type, 
                                                      args.pooling_type, 
                                                      args.vocab_size, 
                                                      args.max_seq_len, 
    ...
    def forward(self, input1: Tensor, input2: Tensor) -> Tensor:
        ...
        encoded1 = self.fnet(embeded1)
        encoded2 = self.fnet(embeded2)
